...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const res = await api.get("/reports/summary"); // ✅ baseURL + JWT handled
        setReport(res.data);
      } catch (err) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-blue-50 p-4 rounded-lg text-center border-l-4 border-blue-500">
          ...
          <p className="text-xl font-bold text-green-600">
            {report.totals.totalProfitKsh >= 0 ? "+" : ""}
            {report.totals.totalProfitKsh.toFixed(2)}
          </p>
        </div>
          ...
          <tbody className="divide-y">
            {report.items.map((item) => (
              ...
              >
                ...
                >
                  {item.profitKsh >= 0 ? "+" : ""}Ksh{" "}
                  {item.profitKsh.toFixed(2)}
                </td>
                <td className="px-2 py-2 text-right">
                  Ksh {item.stockValueKsh.toFixed(2)}
                </td>
              </tr>
            ))}
          </tbody>
